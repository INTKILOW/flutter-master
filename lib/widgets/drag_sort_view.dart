...
class DragSortView extends StatefulWidget {
  ...
  DragSortView(
    ...
    this.onDragListener,
    this.maxCount = 9, this.spaceCount = 3,
  }) : super(key: key);
...
class DragSortViewState extends State<DragSortView>
  ...
  void _init(BuildContext context, EdgeInsets padding, EdgeInsets margin) {
    ...
    _itemWidth = (width - space * 2) / widget.spaceCount;
    _positions.clear();
    for (int i = 0; i < widget.maxCount; i++) {
      double left = (space + _itemWidth) * (i % widget.spaceCount);
      double top = (space + _itemWidth) * (i ~/ widget.spaceCount);
  ...
  int _checkIndexTop(Rect other) {
    int index = -1;
    double? area;
    for (int i = 0; (i < widget.spaceCount && i < _itemCount); i++) {
      Rect rect = _positions[i];
  ...
  int _checkIndexBottom(Rect other) {
    ...
    if (tagIndex != -1) {
      for (int i = _itemCount - 1; i >= 0; i--) {
        if (((i + 1) / widget.spaceCount).ceil() >= (((_dragIndex + 1) / widget.spaceCount).ceil()) &&
            (i % widget.spaceCount == tagIndex)) {
          return i;
        }
      }
    }
  ...
  void _clearAll() {
    _removeOverlay();
    _cacheData.clear();
    int count = math.min(widget.maxCount, widget.data.length);
  ...
  Widget build(BuildContext context) {
    _itemCount = math.min(widget.maxCount, widget.data.length);
    ...
    int column = (_itemCount > widget.spaceCount ? widget.spaceCount : _itemCount + 1);
    int row =
        ((_itemCount + (_itemCount < widget.maxCount ? 1 : 0)) / widget.spaceCount).ceil();
    double realWidth =
